...
import {
  ...
  Search, DollarSign, Boxes, AlertTriangle,
} from 'lucide-react';
...
export default function ProductsManager() {
  ...
  const handleEdit = (product) => {
    ...
    setFormData({
      ...
      unidades_por_bandeja: product.unidades_por_bandeja || '',
    });
...
console.log(products)
  ...
  const totalInventario = products.reduce((acc, p) => acc + Number(p.unidades_por_bandeja || 0), 0);
  const valorInventario = products.reduce((acc, p) => acc + Number(p.precio || 0), 0);
    ...
    <div style={{ padding: '32px' }} className="flex-1 bg-gray-50">
      ...
      <div style={{ marginBottom: '32px' }} className="grid grid-cols-2 lg:grid-cols-3 gap-4">
        ...
        >
          ...
          <div>
            ...
            <p style={{ marginTop: '3px', fontSize: '10px', color: 'rgba(255,255,255,0.5)' }}>Productos registrados</p>
          </div>
        </div>
        ...
        >
          <div style={{ margin: '12px 0 12px 14px' }} className="bg-[#13152280] ring-2 ring-cyan-500/30 w-8 h-8 rounded-lg flex items-center justify-center shrink-0">
            <Boxes size={15} className="text-cyan-300" />
          </div>
          <div>
            <p style={{ fontSize: '18px', fontWeight: 800, color: 'white', lineHeight: 1 }}>{totalInventario}</p>
            <p style={{ marginTop: '3px', fontSize: '10px', color: 'rgba(255,255,255,0.5)' }}>Und. bandejas</p>
          </div>
        </div>
        ...
        >
          ...
          <div>
            <p style={{ fontSize: '17px', fontWeight: 800, color: 'white', lineHeight: 1 }}>${valorInventario.toLocaleString('es-CO')}</p>
            <p style={{ marginTop: '3px', fontSize: '10px', color: 'rgba(255,255,255,0.5)' }}>Valor inventario</p>
          </div>
        </div>
      </div>
        ...
        <div style={{ padding: '24px' }} className="bg-white border border-slate-200 rounded-2xl shadow-sm h-fit sticky top-6">
          ...
          <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
            ...
            <div>
              ...
              <input
                type="number" name="precio" placeholder="$ 0"
            ...
            <div>
              ...
              <input
                type="number" name="unidades_por_bandeja" placeholder="0"
                value={formData.unidades_por_bandeja} onChange={handleChange} required
                style={{ width: '100%', border: '1px solid #e2e8f0', borderRadius: '10px', padding: '10px 14px', fontSize: '14px', outline: 'none', boxSizing: 'border-box' }}
                onFocus={e => { e.target.style.borderColor = '#6366f1'; e.target.style.boxShadow = '0 0 0 3px rgba(99,102,241,0.1)' }}
                onBlur={e => { e.target.style.borderColor = '#e2e8f0'; e.target.style.boxShadow = 'none' }}
              />
            ...
            <div style={{ display: 'flex', gap: '12px', marginTop: '4px' }}>
              ...
              {editingId && (
                <button
                  type="button" onClick={() => { setEditingId(null); setFormData(FORM_INICIAL); }}
              ...
              <tbody>
                {filteredProducts.length === 0 ? (
                  ...
                  filteredProducts.map((product) => (
                    ...
                    >
                      <td style={{ padding: '12px 16px', fontWeight: 600, color: '#334155', fontSize: '14px' }}>{product.nombre}</td>
                      <td style={{ padding: '12px 16px', color: '#64748b', fontSize: '14px' }}>{product.descripcion || '—'}</td>
                      <td style={{ padding: '12px 16px', textAlign: 'right', fontWeight: 700, color: '#334155', fontSize: '14px' }}>
                        ${Number(product.precio).toLocaleString('es-CO')}
                      </td>
                      <td style={{ padding: '12px 16px', textAlign: 'center' }}>
                        <span style={{ background: '#fef3c7', color: '#92400e', padding: '3px 10px', borderRadius: '20px', fontSize: '12px', fontWeight: 700 }}>
                          {product.unidades_por_bandeja} und
                        </span>
                      </td>
      ...
      {deleteId && (
        <div style={{ padding: '16px' }} className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50">
          <div style={{ width: '100%', maxWidth: '400px', borderRadius: '20px', overflow: 'hidden', boxShadow: '0 25px 50px rgba(0,0,0,0.25)', padding: '32px', textAlign: 'center' }} className="bg-white">
